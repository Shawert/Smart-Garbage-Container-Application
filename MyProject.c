...
Lcd_init();
ADC_init();
ANSEL  = 0x03;              // AN0 ve AN1 pini analog olarak ayarlanýyor.
ANSELH = 0;                 // Diðer pinler dijital olarak ayarlanýyor
C1ON_bit = 0;               // Karsilastiricilar kapali
C2ON_bit = 0;
TRISB = 0x00;               // B portu çýkýþ olarak tanýmlanýyor

PORTB.B1 = 1;               //Yeþil led
PORTB.B2 = 0;               //Kýrmýzý led
PORTB.B3 =0;                 //Buzzer
INTCON.GIE=1; // Tüm kesmeler aktif.
INTCON.INTE=1; // RB0/INT dýþ kesme biti
OPTION_REG.INTEDG=0; // Interrupt kenarý seçme biti 1: Yükselen, 0: Düþen
